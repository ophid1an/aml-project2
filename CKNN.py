    ...
    def DistanceMatrix(self, bags):
        ...
        for bag in bags:
            ...
            for i in range(0, len(bags)):
                BagDistances[i] = max(directed_hausdorff(bags[i].toarray(), bag.toarray())[0],
                                      directed_hausdorff(bag.toarray(), bags[i].toarray())[0])
...
#                              min((min([list(dist.euclidean(x.toarray(), y.toarray()) for x in X) for y in
#                                        Y])))) if sp.issparse(X) else max(
