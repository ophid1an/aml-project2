...
def split_train_test(X, y, test_size, n_labeled):
    ...
    X_train, X_test, y_train, y_ = \
        train_test_split(X_, y, test_size=test_size, random_state=RANDOM_STATE)
...
qs = UncertaintySampling(trn_ds, method='lc', model=SVM(random_state=RANDOM_STATE))
model = SVM(random_state=RANDOM_STATE)
E_in_1, E_out_1 = run(trn_ds, tst_ds, lbr, model, qs, SAMPLES_NUM)

qs2 = RandomSampling(trn_ds2, random_state=RANDOM_STATE)
model = SVM(random_state=RANDOM_STATE)
